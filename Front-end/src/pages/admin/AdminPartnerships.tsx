...
export default function AdminPartnerships() {
  ...
  function renderAgreementActions(agreement: Agreement) {
    ...
    if (agreement.status === "pending" && isPartner) {
      return (
        <>
          <Button size="sm" variant="outline" onClick={() => updateAgreementStatus(agreement.id, "active")}>
            <CheckCircle2 className="w-4 h-4 mr-1" /> Aceitar
          </Button>
          <Button size="sm" variant="outline" onClick={() => updateAgreementStatus(agreement.id, "rejected")}>
    ...
    if (agreement.status === "pending" && isRequester) {
      return <Button size="sm" variant="outline" onClick={() => updateAgreementStatus(agreement.id, "canceled")}>Cancelar convite</Button>;
    }

    if (agreement.status === "active") {
      return <Button size="sm" variant="outline" onClick={() => updateAgreementStatus(agreement.id, "inactive")}>Inativar</Button>;
    }

    return <span className="text-xs text-muted-foreground">Sem ações disponíveis</span>;
  }

  function agreementSubtitle(agreement: Agreement) {
    const { isRequester, isPartner } = getAgreementDirection(agreement);

    if (agreement.status === "pending" && isRequester) return "Convite enviado · aguardando resposta da cidade parceira";
    if (agreement.status === "pending" && isPartner) return "Convite recebido · aceite ou rejeite a solicitação";
    if (agreement.status === "active") return "Parceria ativa";
    if (agreement.status === "rejected") return "Convite rejeitado";
    if (agreement.status === "canceled") return "Convite cancelado";
    if (agreement.status === "inactive") return "Parceria inativa";
                  ...
                  {agreements.map((agreement) => (
                    <TableRow key={agreement.id}>
                      <TableCell>
                        ...
                        <p className="text-xs text-muted-foreground">{agreementSubtitle(agreement)}</p>
                      </TableCell>
                      ...
                      <TableCell className="text-right"><div className="flex justify-end gap-2">{renderAgreementActions(agreement)}</div></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
