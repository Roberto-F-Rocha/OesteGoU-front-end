      ...
      <div className="bg-card border border-border rounded-xl p-3">
        <div className="relative max-w-md">
          ...
          <Input
            ...
            placeholder="Buscar aluno, e-mail ou instituição..."
            className="pl-9"
          />
        </div>
      </div>
                ...
                <TableRow key={student.id}>
                  <TableCell>
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">CPF: {student.cpf ?? "Não informado"}</p>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div>
                      <p className="text-foreground">{student.institution ?? "Não informado"}</p>
                      <p className="text-xs text-muted-foreground">{student.city?.name ?? adminCity}</p>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div>
                      <p className="text-sm text-foreground">{student.email}</p>
                      <p className="text-xs text-muted-foreground">{student.phone ?? "Telefone não informado"}</p>
                    </div>
                  </TableCell>
                  <TableCell>
                    <Badge variant={student.status === "active" ? "default" : student.status === "blocked" ? "destructive" : "secondary"}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      <Button
                        ...
                        onClick={() => updateStatus(student.id, student.status === "active" ? "pending" : "active")}
                        title={student.status === "active" ? "Marcar como pendente" : "Ativar aluno"}
                      >
                      ...
                      <Button
                        ...
                        size="sm"
                        disabled={updatingId === student.id}
                        onClick={() => updateStatus(student.id, student.status === "blocked" ? "active" : "blocked")}
                        title={student.status === "blocked" ? "Desbloquear" : "Bloquear aluno"}
                      >
