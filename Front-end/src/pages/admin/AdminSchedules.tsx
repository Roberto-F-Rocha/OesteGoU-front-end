...
export default function AdminSchedules({ adminCity, adminState }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.id) {
        const currentRoute = routes.find((r) => r.id === form.id);
        ...
        scheduleId = currentRoute?.schedule.id ?? 0;
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        {loading ? <div className="p-8 text-center text-sm text-muted-foreground">Carregando horários...</div> : orderedRoutes.length === 0 ? <div className="p-8 text-center text-sm text-muted-foreground">Nenhuma rota cadastrada ainda.</div> : (
          <div className="w-full overflow-x-auto">
            <Table className="min-w-[920px]"><TableHeader><TableRow><TableHead>Rota</TableHead><TableHead>Horário</TableHead><TableHead>Universidade</TableHead><TableHead>Motorista</TableHead><TableHead>Veículo</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Ações</TableHead></TableRow></TableHeader><TableBody>{orderedRoutes.map((route) => (
