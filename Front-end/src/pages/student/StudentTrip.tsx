...
interface Reservation {
  ...
  route?: { id: number; name: string; city?: { name: string; state: string } | null; vehicle?: { name?: string | null; plate: string; capacity: number } | null; driver?: { nome: string; email: string } | null; } | null;
...
export default function StudentTrip() {
  ...
  const displayReservations = useMemo(() => reservations.filter((reservation) => reservation.status !== "absent"), [reservations]);
  const groupedReservations = useMemo(() => { const grouped: Record<string, Reservation[]> = Object.fromEntries(DAYS.map((day) => [day, []])); displayReservations.forEach((reservation) => { const day = reservation.dayOfWeek || "Sem dia"; if (!grouped[day]) grouped[day] = []; grouped[day].push(reservation); }); Object.values(grouped).forEach((items) => items.sort((a, b) => (a.schedule?.time ?? "").localeCompare(b.schedule?.time ?? ""))); return grouped; }, [displayReservations]);
  ...
  async function loadData(showLoader = true) { try { if (showLoader) setLoading(true); const [reservationsResponse, passengersResponse] = await Promise.all([api.get("/my-reservations"), api.get("/students/my-trip-passengers")]); setReservations(reservationsResponse.data ?? []); setPassengers(passengersResponse.data ?? []); } catch { toast({ title: "Erro ao carregar viagem", description: "Não foi possível buscar suas reservas agora.", variant: "destructive" }); } finally { setLoading(false); } }
  useEffect(() => { loadData(); }, []);
  useEffect(() => { const refresh = () => loadData(false); socket.on("reservation:created", refresh); socket.on("reservation:canceled", refresh); socket.on("route:occupancy-updated", refresh); return () => { socket.off("reservation:created", refresh); socket.off("reservation:canceled", refresh); socket.off("route:occupancy-updated", refresh); }; }, []);
  const handleConfirm = async (reservation: Reservation) => { if (reservation.status === "confirmed") { toast({ title: "Presença já confirmada", description: `${tripLabel(reservation.schedule?.type)} já está marcada como vou.` }); return; } try { setActionLoading(reservation.id); await api.patch(`/reservations/${reservation.id}/confirm`); toast({ title: "Presença confirmada", description: `${tripLabel(reservation.schedule?.type)} confirmada novamente.` }); await loadData(false); } catch (error: any) { toast({ title: "Erro ao confirmar", description: error?.response?.data?.error ?? "Não foi possível confirmar esta viagem.", variant: "destructive" }); } finally { setActionLoading(null); } };
  const handleCancel = async (reservation: Reservation) => { if (reservation.status !== "confirmed") { toast({ title: "Ausência já registrada", description: `${tripLabel(reservation.schedule?.type)} já está marcada como não vou.` }); return; } try { setActionLoading(reservation.id); await api.patch(`/reservations/${reservation.id}/cancel`); toast({ title: "Ausência registrada", description: `${tripLabel(reservation.schedule?.type)} marcada como não vou.` }); await loadData(false); } catch { toast({ title: "Erro ao cancelar", description: "Não foi possível registrar ausência nesta viagem.", variant: "destructive" }); } finally { setActionLoading(null); } };
