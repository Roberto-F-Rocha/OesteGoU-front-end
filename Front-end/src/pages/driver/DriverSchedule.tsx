...
function getRouteDay(route: DriverRoute) {
  return route.schedule?.type === "volta" ? "Volta" : "Ida";
}
...
export default function DriverSchedule() {
    ...
    async function loadRoutes() {
      try {
        ...
        toast({
          ...
          description: "Não foi possível buscar suas rotas agora.",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
  ...
  const filteredRoutes = useMemo(() => {
    return routes.filter((route) => {
      const universityName = route.schedule?.university?.name ?? route.name;
      const matchSearch = search.trim()
        ? universityName.toLowerCase().includes(search.toLowerCase()) || route.name.toLowerCase().includes(search.toLowerCase())
        : true;
      const matchDay = filterDay === "Todos" ? true : getRouteDay(route) === filterDay;
      return matchSearch && matchDay;
    });
  }, [routes, search, filterDay]);

  const grouped = ["Ida", "Volta"].map((day) => ({
    day,
    items: filteredRoutes.filter((route) => getRouteDay(route) === day),
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm">
          Sua escala definida pela administração de {city}.
        </p>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl p-3 flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Buscar por rota ou universidade..."
            value={search}
      ...
      {loading ? (
        ...
        <div className="bg-card border border-dashed border-border rounded-xl p-10 text-center">
          ...
          <p className="font-heading font-semibold text-foreground">
            {routes.length === 0 ? "Nenhuma escala atribuída" : "Nenhum resultado"}
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {grouped.map(({ day, items }, dayIdx) => (
            ...
            >
              ...
              {items.map((route) => {
                const shift = getShift(route.schedule?.time ?? "00:00");
                const ShiftIcon = shift.icon;
                const point = route.points?.[0]?.pickupPoint;
                return (
                  <div key={route.id} className="space-y-3 pt-3 border-t border-border first:border-t-0 first:pt-0">
                    <div className="flex items-start justify-between gap-2">
                      <p className="font-heading font-semibold text-foreground flex items-center gap-2">
                        <Bus className="w-4 h-4 text-primary" /> {route.schedule?.university?.name ?? route.name}
                      </p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                      <div className="rounded-lg bg-muted/40 p-2.5">
                        ...
                        <p className="text-foreground font-medium">{route.schedule?.time ?? "--:--"}</p>
                        <p className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5">
                          <MapPin className="w-3 h-3" /> {point?.name ?? route.name}
                        </p>
                      </div>
                      <div className="rounded-lg bg-muted/40 p-2.5">
                        <div className="flex items-center gap-1.5 text-muted-foreground text-xs mb-1">
                          <Users className="w-3.5 h-3.5 text-accent" /> Passageiros
                        </div>
                        <p className="text-foreground font-medium">{route.reservations?.length ?? 0} confirmados</p>
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {route.vehicle ? `${route.vehicle.name ?? "Veículo"} - ${route.vehicle.plate}` : "Veículo não informado"}
                        </p>
                      </div>
                    </div>
                  </div>
