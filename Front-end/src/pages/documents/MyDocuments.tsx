...
import { CheckCircle2, FileText, Image, Loader2, Upload, XCircle, Clock3, AlertCircle } from "lucide-react";
...
export default function MyDocuments() {
  ...
  const latestByType = useMemo(() => {
    ...
    documents.forEach((document) => {
      ...
      if (!current || new Date(document.createdAt) > new Date(current.createdAt)) {
        grouped.set(document.type, document);
      }
    });
    return grouped;
  }, [documents]);
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-4">
        <form onSubmit={handleUpload} className="bg-card border border-border rounded-xl p-4 space-y-4">
              ...
              {options.map((option) => {
                ...
                return (
                  <button
                    key={option.value}
                    type="button"
                    onClick={() => setType(option.value as DocumentType)}
                    className={cn(
                      "rounded-xl border p-3 text-left transition-all bg-background hover:border-primary/40",
          ...
          <div className="space-y-2">
            ...
            <button
              ...
              className={cn(
                "w-full rounded-2xl border-2 border-dashed p-6 text-center transition-all bg-background",
            ...
            >
              ...
              {file ? (
                <div className="space-y-3">
                  {previewUrl ? (
                    <img src={previewUrl} alt="Prévia do documento" className="mx-auto h-28 w-28 rounded-xl object-cover border border-border" />
                  ) : (
                    <div className="mx-auto h-16 w-16 rounded-xl bg-primary/10 flex items-center justify-center"><FileText className="w-8 h-8 text-primary" /></div>
                  )}
                  <div>
                    <p className="font-medium text-foreground break-all">{file.name}</p>
                    <p className="text-xs text-muted-foreground">{formatBytes(file.size)}</p>
                  </div>
                  <span className="text-xs text-primary font-medium">Clique para trocar o arquivo</span>
                </div>
              ) : (
                <div className="space-y-2">
                  <div className="mx-auto h-14 w-14 rounded-2xl bg-primary/10 flex items-center justify-center"><Upload className="w-7 h-7 text-primary" /></div>
                  <p className="font-heading font-semibold text-foreground">Clique para escolher ou arraste o arquivo aqui</p>
                  <p className="text-xs text-muted-foreground">PDF, PNG, JPG, JPEG ou WEBP até 8 MB.</p>
                </div>
              )}
            </button>
          </div>
          ...
          {file && (
            <div className="rounded-xl border border-border bg-muted/30 p-3 flex items-start gap-2 text-sm text-muted-foreground">
              <AlertCircle className="w-4 h-4 mt-0.5 shrink-0" />
              <p>Você está enviando <strong className="text-foreground">{file.name}</strong> como <strong className="text-foreground">{selectedType?.label}</strong>.</p>
            </div>
          )}

          <div className="flex flex-col sm:flex-row gap-2">
            <Button type="submit" disabled={uploading || !file} className="w-full sm:w-auto">
              {uploading ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Upload className="w-4 h-4 mr-2" />} {uploading ? "Enviando..." : "Enviar documento"}
            </Button>
        ...
        <aside className="bg-card border border-border rounded-xl p-4 space-y-3 h-fit">
          ...
          <div className="space-y-3 text-sm text-muted-foreground">
            <p>Confira se o documento está legível antes de enviar.</p>
            <p>Fotos devem mostrar o documento inteiro, sem cortes.</p>
            <p>Após o envio, a administração poderá aprovar ou rejeitar o arquivo.</p>
          </div>
        </aside>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <div className="p-4 border-b border-border flex items-center justify-between gap-3">
          <h2 className="font-heading font-semibold text-foreground">Histórico</h2>
          <span className="text-xs text-muted-foreground">{documents.length} documento{documents.length === 1 ? "" : "s"}</span>
        </div>
        {loading ? <div className="p-6 text-sm text-muted-foreground text-center flex items-center justify-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Carregando...</div> : documents.length === 0 ? <div className="p-8 text-sm text-muted-foreground text-center"><Image className="w-10 h-10 mx-auto mb-3" /> Nenhum documento enviado ainda.</div> : (
          <div className="divide-y divide-border">
            {documents.map((document) => {
              const meta = statusMeta(document.status);
              const Icon = meta.icon;
              return (
                <div key={document.id} className="p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div className="min-w-0 flex items-start gap-3">
                    <div className="h-10 w-10 rounded-xl bg-muted/50 flex items-center justify-center shrink-0"><FileText className="w-5 h-5 text-primary" /></div>
                    <div className="min-w-0">
                      <p className="font-medium text-foreground break-all">{document.fileName}</p>
                      <p className="text-xs text-muted-foreground">{documentLabel(document.type)} · {formatBytes(document.sizeBytes)} · {new Date(document.createdAt).toLocaleString("pt-BR")}</p>
                    </div>
                  </div>
                  <span className={cn("inline-flex items-center gap-1 text-xs font-medium rounded-full border px-2 py-1 w-fit", meta.className)}><Icon className="w-3.5 h-3.5" /> {meta.label}</span>
                </div>
              );
            })}
          </div>
