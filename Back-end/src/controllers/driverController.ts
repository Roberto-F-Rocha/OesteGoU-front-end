...
function ensureDriverOrAdmin(req, res) {
  if (req.user?.role !== "driver" && req.user?.role !== "admin") {
    res.status(403).json({
      error: "Acesso permitido apenas para motoristas e administradores",
    });
    return false;
  }
...
export async function notifyDriverPendingStudents(req, res) {
  ...
  const { userIds, message, trip, targetMode, routeId, dayOfWeek, shift } =
    req.body ?? {};
  ...
  const reservations = candidateReservations.filter((reservation) => {
    if (shift && shiftFromTime(reservation.route?.schedule?.time) !== shift) {
      return false;
    }
  ...
  if (allowedStudentIds.length === 0) {
    return res.status(404).json({
      error: "Nenhum aluno encontrado para essa rota, dia e turno",
    });
  }
  ...
  const universityName =
    first?.route?.schedule?.university?.name ?? "universidade não informada";

  const vehicleName =
    first?.route?.vehicle?.name ||
    first?.route?.vehicle?.plate ||
  ...
  const studentNames = reservations
    .map((reservation) => {
  ...
  } às ${scheduleTime} (${universityName})${
    dayOfWeek ? ` no dia ${dayOfWeek}` : ""
  }. Mensagem enviada: "${cleanMessage}"`;
