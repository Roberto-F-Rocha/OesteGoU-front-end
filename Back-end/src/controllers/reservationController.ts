...
async function buildRouteLivePayload(routeId: number, reservationId?: number) {
  ...
  return {
    ...
    scheduleType: route.schedule?.type,
    scheduleTime: route.schedule?.time,
    shift: shiftFromTime(route.schedule?.time),
    capacity,
    currentCount: confirmedCount,
    overcapacity: Boolean(capacity && confirmedCount > capacity),
...
async function emitOccupancy(routeId, reservationId, scheduleId, dayOfWeek, status = "confirmed") {
  ...
  if (!payload) return;
  const occupancyPayload = { reservationId, routeId, cityId: payload.cityId, scheduleId, dayOfWeek, status, capacity: payload.capacity, currentCount: payload.currentCount, overcapacity: payload.overcapacity };
...
async function notifyDriverAttendanceChange(reservation, action: "confirmed" | "canceled") {
  ...
  await notifyUser(driverId, isConfirmed ? "Presença confirmada" : "Ausência informada", message, isConfirmed ? NotificationType.success : NotificationType.warning, { source: "student_attendance", reservationId: reservation.id, routeId: reservation.routeId, studentId: reservation.userId, status: action, dayOfWeek: reservation.dayOfWeek, scheduleTime: schedule?.time, tripType: schedule?.type });
...
async function notifyRouteOvercapacity({ route, schedule, reservation, dayOfWeek, currentCount, capacity }) {
  ...
  await notifyUsers(recipients, "Ônibus superlotado", `${dayOfWeek ?? "Dia selecionado"}: a ${tripLabel(schedule.type)} às ${schedule.time} ultrapassou a capacidade do ônibus (${currentCount}/${capacity}). A reserva foi mantida para dar visibilidade ao problema.`, NotificationType.warning, payload);
...
export async function createReservation(req, res) {
  ...
  if (inactiveExisting) { req.params.id = String(inactiveExisting.id); return confirmReservation(req, res); }
  const reservation = await prisma.reservation.create({ data: { userId: user.id, scheduleId, routeId, pickupPointId, dayOfWeek, status: "confirmed" }, include: { user: true, schedule: { include: { university: true } }, route: { include: { city: true, vehicle: true, driver: true } }, pickupPoint: true } });
  const capacity = route?.vehicle?.capacity ?? null;
...
export async function createRoundTripReservation(req, res) {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      const activeExistingForDay = await tx.reservation.findFirst({ where: { userId: user.id, dayOfWeek, status: "confirmed" }, include: { schedule: true } });
      ...
      const goingReservation = await tx.reservation.create({ data: { userId: user.id, scheduleId: going.scheduleId, routeId: going.routeId, pickupPointId: goingData.pickupPointId, dayOfWeek, status: "confirmed" }, include: { schedule: { include: { university: true } }, route: { include: { city: true, vehicle: true, driver: true } }, pickupPoint: true } });
