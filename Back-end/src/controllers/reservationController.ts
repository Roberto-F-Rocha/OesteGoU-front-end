...
async function notifyUsers(
  ...
  const uniqueUserIds = Array.from(
    new Set(
      userIds.filter((userId): userId is number => typeof userId === "number"),
    ),
  );

  if (uniqueUserIds.length === 0) return;

  await notifyManyUsers(uniqueUserIds, {
    title,
    message,
    type,
    metadata,
...
async function emitOccupancy(
  ...
  const payload = {
    ...
    capacity,
    currentCount,
    overcapacity: Boolean(capacity && currentCount > capacity),
  };
...
async function notifyRouteOvercapacity({
  route,
  schedule,
  reservation,
  dayOfWeek,
  ...
  await notifyUsers(
    recipients,
    "Ônibus superlotado",
    `${dayOfWeek ?? "Dia selecionado"}: a ${tripLabel(schedule.type)} às ${
      schedule.time
    } ultrapassou a capacidade do ônibus (${currentCount}/${capacity}). A reserva foi mantida para dar visibilidade ao problema.`,
...
async function validateReservationInput(tx, user, input, expectedType) {
  ...
  if (schedule.type !== expectedType) {
    throw new Error(
      expectedType === "ida"
        ? "Selecione uma rota de ida válida"
        : "Selecione uma rota de volta válida",
    );
  }
  ...
  if (route.scheduleId !== schedule.id) {
    throw new Error("A rota não pertence ao horário informado");
  }
  ...
  if (route.cityId !== user.cityId) {
    ...
    if (!agreement) {
      throw new Error("Sua cidade não possui vínculo ativo com a cidade desta rota");
    }
  }

  const activePoints = route.points.filter(
    (item) => item.pickupPoint?.active !== false,
  );

  const matchingPoints = activePoints.filter(
    (item) => item.pickupPoint?.type === expectedType,
  );

  if (input.pickupPointId) {
    const routePoint = matchingPoints.find(
      (item) => item.pickupPointId === input.pickupPointId,
    );

    if (!routePoint) {
      throw new Error(
        expectedType === "ida"
          ? "Ponto de ida não pertence à rota"
          : "Ponto de volta não pertence à rota",
      );
    }

    const point = routePoint.pickupPoint;

    if (point.cityId !== user.cityId) {
      throw new Error("Ponto não pertence à sua cidade");
    }

    if (point.type !== schedule.type) {
      throw new Error("Ponto incompatível com o tipo do horário");
    }

    if (
      point.type === "volta" &&
      schedule.universityId &&
      point.universityId !== schedule.universityId
    ) {
      throw new Error("Ponto de volta não pertence à universidade selecionada");
    }
  } else if (matchingPoints.length > 1) {
    throw new Error(
      expectedType === "ida"
        ? "Selecione um ponto de ida"
        : "Selecione um ponto de volta",
    );
  }

  const pickupPointId =
    input.pickupPointId ??
    (matchingPoints.length === 1 ? matchingPoints[0].pickupPointId : null);
...
export async function createReservation(req, res) {
  ...
  if (pickupPointId) {
    ...
    if (point.cityId !== user.cityId) {
      return res.status(403).json({ error: "Ponto não pertence à sua cidade" });
    }

    if (point.type !== schedule.type) {
      return res.status(400).json({ error: "Ponto incompatível com o tipo do horário" });
    }
    ...
    if (
      ...
      return res.status(400).json({
        error: "Ponto de volta não pertence à universidade selecionada",
      });
    }

    if (routeId) {
      const routePoint = route?.points.find(
        (item) => item.pickupPointId === pickupPointId,
      );

      if (!routePoint) {
        return res.status(400).json({ error: "Ponto não pertence à rota" });
      }
    }
  ...
  if (route && route.cityId !== user.cityId) {
    ...
    if (!agreement) {
      return res.status(403).json({
        error: "Sua cidade não possui vínculo ativo com a cidade desta rota",
      });
    }
  }
  ...
  if (activeExisting) {
    return res.status(409).json({
      error: "Você já possui reserva confirmada para este horário",
    });
  }

  const inactiveExisting = await prisma.reservation.findFirst({
    where: {
      userId: user.id,
      scheduleId,
      dayOfWeek: dayOfWeek ?? null,
      status: { in: ["canceled", "absent"] },
    },
  ...
  await notifyUser(
    user.id,
    isOvercapacity
      ? "Horário confirmado com ônibus superlotado"
      : "Horário confirmado",
    `${dayOfWeek ?? "Dia selecionado"}: sua ${tripLabel(
      reservation.schedule.type,
    )} para ${
      reservation.schedule.university?.name ?? "universidade"
    } às ${reservation.schedule.time} foi confirmada.${
      isOvercapacity
        ? ` Atenção: o ônibus está acima da capacidade (${currentCount}/${capacity}).`
        : ""
    }`,
    isOvercapacity ? NotificationType.warning : NotificationType.success,
    {
      reservationId: reservation.id,
      scheduleId,
      routeId,
  ...
  if (route) {
    ...
    await emitOccupancy(route.id, reservation.id, scheduleId, dayOfWeek, "confirmed");
  }

  if (route && capacity && (reachedCapacity || isOvercapacity)) {
    await notifyRouteOvercapacity({
      route,
      schedule,
      reservation,
      dayOfWeek,
  ...
  await createAuditLog({
    ...
    description: isOvercapacity
      ? "Aluno criou reserva acima da capacidade do ônibus"
      : "Aluno criou uma reserva de transporte",
    metadata: {
      scheduleId,
      routeId,
      pickupPointId,
...
export async function createRoundTripReservation(req, res) {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (activeExistingForDay) {
        throw new Error(
          "Você já possui horário confirmado para este dia. Remova o horário atual antes de cadastrar outro.",
        );
      }
      ...
      if (
        goingData.schedule.universityId &&
        returningData.schedule.universityId &&
        goingData.schedule.universityId !== returningData.schedule.universityId
      ) {
        throw new Error("A ida e a volta precisam ser da mesma universidade");
      }

      if (
        shiftFromTime(goingData.schedule.time) !== shift ||
        shiftFromTime(returningData.schedule.time) !== shift
      ) {
    ...
    await notifyUser(
      user.id,
      "Horário confirmado",
      `${dayOfWeek}: sua ida e volta foram confirmadas com sucesso.`,
      NotificationType.success,
      {
        dayOfWeek,
        shift,
        goingReservationId: result.going.id,
        returningReservationId: result.returning.id,
      },
    );

    if (result.going.routeId) {
      await emitOccupancy(
        result.going.routeId,
        result.going.id,
        result.going.scheduleId,
        dayOfWeek,
        "confirmed",
      );
    }

    if (result.returning.routeId) {
      await emitOccupancy(
        result.returning.routeId,
        result.returning.id,
        result.returning.scheduleId,
        dayOfWeek,
        "confirmed",
      );
    }
    ...
    await createAuditLog({
      ...
      action: "create",
      entity: "Reservation",
      entityId: `${result.going.id},${result.returning.id}`,
      description: "Aluno criou reserva de ida e volta",
      metadata: {
        dayOfWeek,
        shift,
        going,
    ...
    return res.status(400).json({
      error: error instanceof Error ? error.message : "Erro ao salvar horário completo",
    });
  }
}
...
export async function confirmReservation(req, res) {
  ...
  if (activeExisting) {
    return res.status(409).json({
      error: "Já existe uma reserva confirmada para este horário",
    });
  }
  ...
  await notifyUser(
    reservation.userId,
    "Presença confirmada",
    `${reservation.dayOfWeek ?? "Dia selecionado"}: sua ${tripLabel(
      reservation.schedule.type,
    )} às ${reservation.schedule.time} foi confirmada novamente.`,
    NotificationType.success,
    {
      reservationId: reservation.id,
      scheduleId: reservation.scheduleId,
  ...
  await createAuditLog({
    ...
    entity: "Reservation",
    entityId: updated.id,
    description: "Reserva confirmada novamente",
    ...getRequestAuditData(req, res),
  });
...
export async function cancelReservation(req, res) {
  ...
  await notifyUser(
    ...
    `${reservation.dayOfWeek ?? "Dia selecionado"}: sua ${tripLabel(
      reservation.schedule.type,
    )} às ${reservation.schedule.time} foi removida da sua semana.`,
    NotificationType.warning,
    {
      reservationId: reservation.id,
      scheduleId: reservation.scheduleId,
