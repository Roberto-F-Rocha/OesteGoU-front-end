...
function isSeverity(value: unknown): value is MaintenanceSeverity {
  return (
    typeof value === "string" &&
    ALLOWED_SEVERITIES.includes(value as MaintenanceSeverity)
  );
}
...
function isStatus(value: unknown): value is MaintenanceStatus {
  return (
    typeof value === "string" &&
    ALLOWED_STATUSES.includes(value as MaintenanceStatus)
  );
}
...
function getPagination(query: any) {
  ...
  const take = Math.min(
    Math.max(requestedTake || DEFAULT_PAGE_SIZE, 1),
    MAX_PAGE_SIZE,
  );
  ...
  await Promise.all(
    admins.map((admin) =>
      notifyUser(
        ...
        `${
          payload.openedBy?.nome ?? "Motorista"
        } abriu um chamado para o ônibus ${
          payload.vehicle?.plate ?? "informado"
        }.`,
  ...
  try {
    ...
    if (user.role !== "driver") {
      return res.status(403).json({
        error: "Apenas motoristas podem abrir chamados",
      });
    }

    if (!user.cityId) {
      return res.status(403).json({
        error: "Usuário sem cidade definida",
      });
    }

    if (!vehicleId || Number.isNaN(vehicleId)) {
      return res.status(400).json({
        error: "Ônibus é obrigatório",
      });
    }
    ...
    if (!validateText(title, MIN_TITLE, MAX_TITLE)) {
      return res.status(400).json({
        error: `Título deve ter entre ${MIN_TITLE} e ${MAX_TITLE} caracteres`,
      });
    }

    if (!validateText(description, MIN_DESCRIPTION, MAX_DESCRIPTION)) {
      return res.status(400).json({
        error: `Descrição deve ter entre ${MIN_DESCRIPTION} e ${MAX_DESCRIPTION} caracteres`,
      });
    }
    ...
    if (vehicle.cityId !== user.cityId) {
      return res.status(403).json({
        error: "Ônibus não pertence à sua cidade",
      });
    }
    ...
    await notifyUser(
      user.id,
      "Chamado criado",
      `Seu chamado para o ônibus ${ticket.vehicle.plate} foi aberto.`,
...
export async function getMyMaintenanceTickets(req, res) {
  try {
    ...
    if (user.role !== "driver") {
      return res.status(403).json({
        error: "Apenas motoristas podem acessar seus chamados",
      });
    }
...
export async function listMaintenanceTickets(req, res) {
  try {
    ...
    if (user.role !== "admin") {
      return res.status(403).json({
        error: "Apenas administradores podem listar chamados",
      });
    }
    ...
    if (req.query.severity) {
      if (!isSeverity(req.query.severity)) {
        return res.status(400).json({
          error: "Gravidade inválida",
        });
      }
  ...
  try {
    ...
    const resolution =
      typeof req.body?.resolution === "string"
        ? sanitizeText(req.body.resolution)
        : undefined;

    if (user.role !== "admin") {
      return res.status(403).json({
        error: "Apenas administradores podem atualizar chamados",
      });
    }
    ...
    if (resolution !== undefined && resolution.length > MAX_DESCRIPTION) {
      return res.status(400).json({
        error: `Resposta deve ter no máximo ${MAX_DESCRIPTION} caracteres`,
      });
    }
    ...
    if (current.cityId !== user.cityId) {
      return res.status(403).json({
        error: "Chamado pertence a outra cidade",
      });
    }
